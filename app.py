...
""", unsafe_allow_html=True)
        ...
        with col_btn1:
            if st.button("💡 AI 전략 생성", type="secondary", use_container_width=True):
        ...
        if st.session_state.show_strategy:
            try:
                with st.spinner("🤖 Gemini가 정책 및 사례를 분석하여 전략을 작성 중..."):
                    strategy, strategy_sources = generate_rag_strategy(
                        risk_score, risk_factors, safe_factors, 
                        업종, 지역, vectorstore, llm, merchant_data
                    )
                ...
                if strategy:
                    ...
                    if strategy_sources:
                        st.markdown("---")
                        st.markdown('<div class="section-header"><h3>📚 참고 자료 (정책 및 사례 문서)</h3></div>', unsafe_allow_html=True)
                        
                        for idx, source in enumerate(strategy_sources, 1):
                            # PDF 파일명 정리
                            display_name = source.replace('.pdf', '').replace('_', ' ')
                    ...
                    st.session_state.strategy_sources = strategy_sources
                else:
                    st.error("전략을 생성할 수 없습니다.")
            
            except Exception as e:
                st.error(f"전략 생성 중 오류 발생: {e}")
                import traceback
                st.code(traceback.format_exc())
        
        # 이미 생성된 전략이 있으면 표시
        elif 'generated_strategy' in st.session_state:
            st.markdown(st.session_state.generated_strategy)
            
            # 이전에 저장된 출처도 표시
            if 'strategy_sources' in st.session_state and st.session_state.strategy_sources:
                st.markdown("---")
                st.markdown('<div class="section-header"><h3>📚 참고 자료 (정책 및 사례 문서)</h3></div>', unsafe_allow_html=True)
                
                for idx, source in enumerate(st.session_state.strategy_sources, 1):
                    display_name = source.replace('.pdf', '').replace('_', ' ')
